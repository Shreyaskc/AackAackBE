<?php
Class new_user_social_model extends CI_Model
{
	...
	//echo 'socialid '.$socialid.'<br/>';
//	echo 'logintype '.$logintype.'<br/>';
//	echo 'deviceid '.$deviceid.'<br/>';
		...
		if(!empty($socialid) && !empty($logintype) && !empty($deviceid))
		{
				...
				$c1 = $q1->num_rows();
				$r1 = $q1->row_array();
				//echo 'Number of rows '.$cl.'<br/>';
				// ======= if social id already exists just give his details
				if($c1 > '0')
				{
					...
					$user = $r2['userid'];
					//echo $user."<br/>";
					$q3 = $this->db->query("select * from tbl_users where `userid` = '$user'");
					$r3 = $q3->row_array();
					if($q3->num_rows() > 0)	
					{
						$userid = $r3['userid'];
						$profile_status = $r3['profile_status'];
							...
							elseif($profile_status == '0')
							{
								...
								$message=array("message"=>"User with this email already exists",'userid'=>$r3['userid'],'username'=>$r3['username'],'email'=>$r3['email'],'profilepic'=>$r3['profile_pic'],
										...
										'fullname'=>$fullname,'followingcount'=>$followingcount,'followerscount'=>$followerscount,'followstatus'=>$followstatus,'aackscount'=>$aackscount,
										'profilestatus'=>'0','lastbackup'=>($backup['lastbackup'] == null)?"":$backup['lastbackup']);
							}
					} 
					else
					{
				...
				else
				{
					...
					$r9 = $q9->row_array();
					$count9 = $q9->num_rows();
					//echo 'device token row count '.$q9.'<br/>';
					$userid3 = $r9['userid']; 
					//echo $count9;
					if($count9 > '0')
					{
							...
							//**********************************************************************
								...
								if($q5)
								{
									...
									if($q6)
									{
										...
										$r7 = $q7->row_array();
												$backuptime=$this->db->query("select lastbackup from tbl_backupmessages where userid='$userid3'");
												$backup = $backuptime->row_array(); 										
										...
										'lastbackup'=>$backup['lastbackup']=($backup['lastbackup'] == null)?"":$backup['lastbackup']); 
									}
									else
									{
										//failed message
					...
					{ 
				//echo ' device id does not exist'.'<br/>';
						//echo "asdasd";
											//echo 'executing ';
					...
					//echo 'executed ';
						if($q4)
						{
							...
							if($q5)
							{
								...
								if($q6)
								{
									//echo 'insert Flag tbl_instagram = true'.'<br/>';
									
									$q7 = $this->db->query("select * from tbl_users where userid = '$insert_id1'");
									$r7 = $q7->row_array();
												$backuptime=$this->db->query("select lastbackup from tbl_backupmessages where userid='$insert_id1'");
												$backup = $backuptime->row_array(); 									
							...
							//echo 'insert Flag tbl_users = false'.'<br/>';
						} 
					} 
