<?php
Class getfollowerslist_model extends CI_Model
{
...
/* 	function getFollowList($userid,$start)
	{
		...
		{
			// following
			$query1 = $this->db->query("select * from tbl_users where userid in (select followee from tbl_follow where follower = '$userid') order by username limit $start,20");
			$res1 = $query1->result();
		...
		// get messages of all users who are following you...
	function getFollowList($userid,$devicedatetime,$start)
	{
		$message=array();
		$message1=array();
		if(!empty($userid))
			...
			if($query1->num_rows() > 0)
			{
				foreach($row1 as $value)
				{
					...
					//echo "select * from tbl_users where userid=$user";
						...
						"conversation"=>$res5['conversation_from'],"likestatus"=>$likestatus,"followstatus"=>"follower"); 
