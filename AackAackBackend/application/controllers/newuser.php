...
class newuser extends CI_Controller
{
		...
		if($logintype == '3')// general login
		{
			if(!empty($image))
			{
				...
				if ($this->upload->do_upload('logo'))
				{
					...
					$results = $this->new_user_model->testaddUserSample($username,$password,$number,$logintype,$firstname,$lastname,$logo,$email,$devicetoken);
				}
			}
			else
			{
	...
	function loginUser()
	{
		...
		if($logintype == '3')// general login
		{
		$results = $this->new_user_model->loginUser($username,$password,$logintype,$devicetoken);
		}
		if($logintype == null)//facebook
		{
		$results = array('message'=>"provide values");
